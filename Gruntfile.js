...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        */
        ...
        cssUrlRewrite: {
            dist: {
                src: '<%= yeoman.dist %>/styles/custom.css',
                dest: '<%= yeoman.dist %>/styles/custom.css',
                options: {
                    skipExternal: true,
                    rewriteUrl: function (url) {
                        var fontEndings = '.[otf|eot|svg|ttf|woff|woff2|png|jpg]',
                            filename = url.split('\\').pop(),
                            filenameReg = new RegExp(fontEndings);
                        if (filenameReg.test(fontEndings)) {
                            var path = filename.split('/');
                            var penult = path[path.length - 2];
